-- creating the /archive endpoint which returns current hidden chats

create or replace view api.archive as
select 
...
where my_p.profile_id = api.auth_profile_id() and my_p.status = 'hidden';

-- grant permission to select this view
grant select on api.archive to authenticated_user;